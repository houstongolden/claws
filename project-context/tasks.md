...
- Proactivity Engine: scheduled jobs, executions, notifications, model policies (runtime-db); gateway API and job runner; dashboard Proactivity page; slash commands /morning-brief, /eod, /watchdog. **Gateway 30s scheduler** runs due jobs via `listDueScheduledJobs` + `runProactiveJob` (cron throttle until full cron parsing).
- Session UX: **Nav Sessions** (starred + recent), resume chat; streaming transcript persist on complete.
- Vibe coding: **file cards** for writes, **artifact panel** (preview + open in browser), **sidebar collapse** on artifact; **aiHandler** steers fs.write vs chat dumps.
- Gateway DB: **PGlite** preferred at `.claws/runtime/`; **in-memory fallback** if persistent init fails (gateway still starts; no durable state across restarts in fallback).
