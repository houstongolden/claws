...
|--------|--------|----------|
| **Session model** | partial | **files:** apps/gateway session handling, runtime-db sessions table, session-workbench.tsx. **routes:** chat carries chatId/threadId. **ui:** Session workbench, chatId sent. **runtime:** Streaming path persists assistant reply after stream complete via `onComplete` → `persistSessionHistory` in apps/gateway/src/main.ts; messages stored in PGlite per session; session list/resume UI optional for v0. **issues:** Session list and resume in UI not yet implemented. |
...
| Feature | Status | Evidence |
|--------|--------|----------|
| **Chat surface** | partial | **files:** session-workbench.tsx, chat page, api/chat. **ui:** Streaming, tool cards, approval CTA, command chips, slash commands. **runtime:** Streaming path persists assistant reply after stream (onComplete → persistSessionHistory in main.ts); server has PGlite messages per session; session list/resume UI optional; tool results at end of stream. **issues:** Session list/resume in UI; incremental tool UI in stream. |
